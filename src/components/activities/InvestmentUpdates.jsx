...
const InvestmentUpdates = () => {
  ...
  return (
    ...
    <h5>Investments</h5>
    <div className='flex-justify-space'><span className='text-xl'>Total Assets Value:</span> <span className='semi-bold text-xl'>{loading ? <Loader/> :  USD.format(user.total_asset)}</span></div>
    <div className='top'>
      <span>Important</span>
    ...
    <div>
      ...
      {loading ? <Loader/> : user.portfolios.map(portfolio => (
        <li className='flex-space p-2'><span className='flex-grid' >{portfolio.asset.title}</span> <span className='flex-grid'>{portfolio.asset.asset_type}</span> <span className='flex-grid semi-bold text-xl'> ${portfolio.amount}</span></li>
      ))}
