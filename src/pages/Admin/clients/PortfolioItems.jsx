...
const PortfolioItems = () => {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(createInterest({ portfolio_id, interest }));
  };
  useEffect(() => {
    ...
    <div className="py-4">
      <div>
        {portfolio.portfolio_interests.length < 1 ? <h2 className="pb-3">No interest yet</h2> : portfolio.portfolio_interests.map((interest) => (
          <div className="box-shadow py-3 my-2 px-2 flex justify-between items-center">
            <span className="font-medium text-lg">interest: </span>
            {' '}
            $
            {interest.interest}
            {' '}
            <span className="font-normal">{interest.created_at}</span>
            {' '}
          </div>
        ))}
      </div>
